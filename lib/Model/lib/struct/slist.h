...
template<typename T>
inline ListNode<T>* SList<T>::insertBetween(ListNode<T>* a, ListNode<T>* b, T t)
{
	if (a&&b) {
...
template<typename T>
inline ListNode<T>* SList<T>::locate(int index)
{
	if (index < -1) {
		return NULL;	   // i不合理?
	}
...
template<typename T>
inline ListNode<T>* SList<T>::getFirst()
{
	return first->link;
}
template<typename T>
inline ListNode<T>* SList<T>::getLast()
{
	return last;
}
;
