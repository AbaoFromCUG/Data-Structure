...
public:
	...
	~AMMap();
    AMMap<D> operator =(const AMMap<D>& old);
    ...
    virtual void updateEdge(int v1,int v2,int cost);
//    void updateEdge(int v1,int v2,T cost)
...
template<typename D>
AMMap<D> AMMap<D>::operator =(const AMMap<D> &old)
{
    this->setTSave(SaveType::AdMatrix);         //邻接矩阵的存储方式
    //将矩阵数组设置为0，即代表没有链接的边
    this->m_iMatrix = new int[this->m_iCapacity*this->m_iCapacity];
...
template<typename D>
void AMMap<D>::updateEdge(int v1, int v2, int cost)
{
    if(v1>this->m_iNodeCount||v1<0){
        throw "error ,the row value is out of range";
    }
