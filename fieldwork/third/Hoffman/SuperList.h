...
public:
	SuperList();
        SuperList(const SuperList<T>& list);
	virtual ~SuperList();
...
template<typename T>
inline SuperList<T>::SuperList(const SuperList<T>& list)
{
	first = new ListNode<T>();
        size = list.getSize();
...
template<typename T>
inline int SuperList<T>::getSize() const
{
	return size;
}
...
template<typename T>
inline ListNode<T>* SuperList<T>::getFirst() const
{
	return first->link;
}
template<typename T>
